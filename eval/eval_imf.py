...
def main()->None:
    ...
    for i,row in enumerate(read_rows(a.test_file)):
        prompt,target=row["text"],row["answer"]; prompt_batch=tok(prompt,return_tensors="pt",add_special_tokens=True); prompt_ids=prompt_batch.input_ids.to(model.device); attention_mask=prompt_batch.attention_mask.to(model.device); full=tok(prompt+target,return_tensors="pt",add_special_tokens=True).input_ids.to(model.device); labels=full.clone(); labels[:,:prompt_ids.shape[1]]=-100
        with torch.inference_mode():
            output=model(full,labels=labels); nll,_=score_target(output.logits,labels)
            generated=model.generate(prompt_ids,attention_mask=attention_mask,do_sample=False,num_beams=1,temperature=None,top_p=None,top_k=None,max_new_tokens=a.max_new_tokens,pad_token_id=tok.eos_token_id)
