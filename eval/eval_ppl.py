...
def _path_stat_payload(path: Path) -> dict:
    if not path.exists():
...
def _model_cache_fingerprint(model_path: str) -> dict:
    path = Path(model_path)
    if not path.exists():
        return {"kind": "remote_or_missing", "model_path": model_path}
    return {
        "kind": "local_dir" if path.is_dir() else "local_file",
        "model_path": str(path),
        "config_json": _path_stat_payload(path / "config.json") if path.is_dir() else _path_stat_payload(path),
        "awq_artifact": _path_stat_payload(path / "awq_artifact.pt") if path.is_dir() else {"exists": False, "mtime_ns": None, "size": None},
    }
# --------------------------------------------------------------------------- #
# CLI
# --------------------------------------------------------------------------- #
...
def main():
    ...
    p.add_argument("--overwrite", action="store_true",
                   help="Ignore a matching cached PPL json and rerun evaluation")
    args = p.parse_args()

    dtype = _DTYPES[args.dtype]
    cache_dir = Path(args.cache_dir) if args.cache_dir else None
    out = Path(args.out_json) if args.out_json else Path(args.model_path) / "ppl.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    eval_manifest_name = out.name + ".manifest.json"
    manifest_params = {
        "model": _model_cache_fingerprint(args.model_path),
    ...
    hit, reason = cache_hit(
        ...
        "eval_ppl",
        manifest_params,
        required_outputs=[out.name],
        overwrite=args.overwrite,
        manifest_name=eval_manifest_name,
    )
    if hit:
        print(f"[cache] Reusing eval json {out}: {reason}", flush=True)
        return
    print(f"[cache] Eval cache miss for {out}: {reason}", flush=True)
    ...
    write_manifest(out.parent, "eval_ppl", manifest_params, manifest_name=eval_manifest_name)
    print("-" * 60)
    print(f"Wrote {out}")
