...
generated=data/imf_generated
PYTHONPATH=src python scripts/generate_imf_dataset.py --validate-only "$generated" || {
  echo "Missing or invalid generated ImF dataset in $generated; run generate_imf_dataset.py first" >&2
  exit 2
}
...
for file in train_stego60.json test_stego10.jsonl stegoY.txt stegoX.txt manifest.json; do
  install -m 0644 "$generated/$file" "data/imf/$file"
done
